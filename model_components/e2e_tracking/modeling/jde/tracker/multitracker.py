...
class JDETracker(object):
    def __init__(self, cfg,weights,iou_thres,nms_thres,conf_thres,min_box_area,track_buffer, img_size,frame_rate=30):

        self.model = Darknet(cfg, nID=14455)
    ...
    def update(self, im_blob, img0):
        ...
        pred = pred[pred[:, :, 4] > self.det_thresh]
        # pred now has lesser number of proposals. Proposals rejected on basis of object confidence score
        if len(pred) > 0:
            dets = non_max_suppression(pred.unsqueeze(0), self.det_thresh, self.nms_thres)[0].cpu()
