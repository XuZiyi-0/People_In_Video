...
class Mars(object):
    ...
    """
    root = '/home/mnx/datasets/MARS-v160809'
    ...
    def __init__(self, min_seq_len=0):
        ...
        train, num_train_tracklets, num_train_pids, num_train_imgs, valid, num_valid_tracklets, num_valid_pids, num_valid_imgs = \
          self._process_data_train(train_names, track_train, home_dir='bbox_train', relabel=True, min_seq_len=min_seq_len)
        ...
        print("  valid    | {:5d} | {:8d}".format(num_valid_pids, num_valid_tracklets))
        ...
        self.num_valid_pids = num_valid_pids
    ...
    def _process_data_train(self, names, meta_data, home_dir=None, relabel=False, min_seq_len=0):
        assert home_dir in ['bbox_train']
        ...
        if relabel: 
            ...
            label = [pid2label[meta_data[i, 2]] for i in range(num_tracklets)]
        tracklets_train = []
        tracklets_valid = []
        num_imgs_per_tracklet_train = []
        num_imgs_per_tracklet_valid = []
        pid_train = []
        pid_valid = []
        
        for p_id in range(num_pids):
            pidx = [i for i, x in enumerate(label) if x == p_id]
            num_t = len(pidx)
            random.shuffle(pidx)
            idx_split = int(num_t/2)
            for i, idx in enumerate(pidx):
                ...
                if len(img_paths) >= min_seq_len:
                    ...
                    if i < idx_split:
                        tracklets_valid.append((img_paths, pid, camid))
                        num_imgs_per_tracklet_valid.append(len(img_paths))
                        pid_valid.append(pid)
                    else:
                        tracklets_train.append((img_paths, pid, camid))
                        num_imgs_per_tracklet_train.append(len(img_paths))
                        pid_train.append(pid)
                    if num_t == 1:
                        tracklets_valid.append((img_paths, pid, camid))
                        num_imgs_per_tracklet_valid.append(len(img_paths))
                        pid_valid.append(pid)
        ...
        for tracklet_idx in range(num_tracklets):
            ...
            if len(img_paths) >= min_seq_len:
                ...
                if random.random() > 0.5:
                    ...
                    pid_valid.append(pid)
            '''
        num_tracklets_train = len(tracklets_train)
        num_tracklets_valid = len(tracklets_valid)
        num_pids_train = len(list(set(pid_train)))
        num_pids_valid = len(list(set(pid_valid)))

        return tracklets_train, num_tracklets_train, num_pids_train, num_imgs_per_tracklet_train, tracklets_valid, num_tracklets_valid, num_pids_valid, num_imgs_per_tracklet_valid
